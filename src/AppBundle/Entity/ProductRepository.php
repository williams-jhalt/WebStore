...
class ProductRepository extends EntityRepository {
    ...
    public function findOrCreate(array $parameters) {
        $entity = $this->findOneBySku($parameters['sku']);

        if (null === $entity) {
            $entity = new Product();
            $entity->setSku($parameters['sku']);
            $entity->setName($parameters['name']);
            $entity->setManufacturer($parameters['manufacturer']);
            $entity->setProductType($parameters['productType']);
